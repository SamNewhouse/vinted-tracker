...
const MarkSoldModal: FC<Props> = ({ bundleId, item, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    dispatch(markItemStatus({ bundleId, itemId: item.id, status: "sold", salePrice: Number(salePrice), saleCosts }));
    onClose();
  };
    ...
    <Modal title="Mark as Sold" subtitle={item.name} onClose={onClose}>
      ...
      <div className="px-6 py-3 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
        <div className="grid grid-cols-3 gap-3 text-center">
          <CostCell label="Item cost" value={formatCurrency(item.allocatedCost + item.extraCostsShare)} colour="muted" />
      ...
      <form onSubmit={handleSubmit}>
        <div className="px-6 py-5 space-y-5">
          <Input
            label="Sale Price"
            type="number" step="0.01" min="0.01" placeholder="0.00" prefix="£"
            value={salePrice}
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-3">
              Sale Costs{" "}
              <span className="font-normal normal-case tracking-normal">(optional — deducted from profit)</span>
            </p>
            <div className="grid grid-cols-2 gap-3">
              <Input label="Postage out" type="number" step="0.01" min="0" placeholder="0.00" prefix="£"
                value={saleCosts.postageOut || ""} onChange={(e) => setCost("postageOut", e.target.value)} hint="Shipping label cost" />
              <Input label="Packaging" type="number" step="0.01" min="0" placeholder="0.00" prefix="£"
                value={saleCosts.packaging || ""} onChange={(e) => setCost("packaging", e.target.value)} hint="Box, bag, bubble wrap" />
              <Input label="Platform fee" type="number" step="0.01" min="0" placeholder="0.00" prefix="£"
                value={saleCosts.platformFee || ""} onChange={(e) => setCost("platformFee", e.target.value)} hint="Vinted, eBay, Depop fee" />
              <Input label="Other" type="number" step="0.01" min="0" placeholder="0.00" prefix="£"
                value={saleCosts.otherCosts || ""} onChange={(e) => setCost("otherCosts", e.target.value)} hint="Any other sale cost" />
            </div>
          </div>
            ...
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
              <div className="grid grid-cols-3 divide-x divide-slate-100 dark:divide-slate-800">
                <div className="px-4 py-3 text-center">
                  <CostCell label="Sale costs" value={formatCurrency(totalSaleCosts)} colour="muted" />
                </div>
                <div className="px-4 py-3 text-center">
                ...
                <div className="px-4 py-3 text-center">
                  ...
                  {profit !== null ? <ProfitValue value={profit} size="base" /> : <span className="text-sm text-slate-400">—</span>}
                </div>
              </div>
            </div>
          )}
