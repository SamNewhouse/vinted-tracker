...
export const selectBundleSummary = (bundle: Bundle, items: Item[]): BundleSummary => {
  ...
  const totalInvested =
    bundle.purchaseCost + bundle.extraCosts.reduce((s, c) => s + c.amount, 0)
    ...
    const avgProfitPerItem =
      soldItems.length > 0
        ? soldItems.reduce((s, i) => {
            ...
            return s + (i.salePrice ?? 0) - i.allocatedPurchaseCost - i.allocatedExtraCostShare - saleCosts
          }, 0) / soldItems.length
        : 0
    ...
    result.sort((a, b) => {
      ...
      switch (filters.sortField) {
        ...
        case "name":
          return a.name.localeCompare(b.name) * dir
        case "profit": {
          const pa = a.salePrice != null
            ? a.salePrice - a.allocatedPurchaseCost - a.allocatedExtraCostShare - a.saleCosts.reduce((s, c) => s + c.amount, 0)
            : -Infinity
          const pb = b.salePrice != null
            ? b.salePrice - b.allocatedPurchaseCost - b.allocatedExtraCostShare - b.saleCosts.reduce((s, c) => s + c.amount, 0)
            : -Infinity
          return (pa - pb) * dir
        }
        default:
